...
class _Widget(QtWidgets.QWidget):
    def __init__(self, data_file):
        ...
        self.room = self.read_coordinates(data_file)

        # parse coordinates from room
        self.src_x = self.room['src_x']
        self.src_y = self.room['src_y']
        self.src_z = self.room['src_z']

        self.mic_x = self.room['mic_x']
        self.mic_y = self.room['mic_y']
        self.mic_z = self.room['mic_z']

        # plot figure, sources and microphones
        self.fig_room = plt.figure()
        self.ax = plt.axes(projection='3d')
        self.ax.scatter(self.src_x, self.src_y, self.src_z, zdir='z', c=None, depthshade=False, marker='x')
        self.ax.scatter(self.mic_x, self.mic_y, self.mic_z, zdir='z', c=None, depthshade=False, marker='o')

        self.ax.set_xlim([0, self.room['room_coordinates'][0] + 5])
        self.ax.set_ylim([0, self.room['room_coordinates'][1] + 5])
        self.ax.set_zlim([0, self.room['room_coordinates'][2] + 5])

        self.fig_room.patch.set_alpha(0)
    ...
    def update(self, file):
        # updates room 3D scatter

        # read coordinates of the space
        self.room = self.read_coordinates(file)
    ...
    def read_coordinates(self, file=None):
        # write coordinates from .yaml file, write into room
        ...
        self.src_y = []
        self.src_z = []
        for i in d['Sources']['ID']:
            source_coordinates = [d['Sources']['sources'][i]['functional_form']['x'],
                                  d['Sources']['sources'][i]['functional_form']['y'],
                                  d['Sources']['sources'][i]['functional_form']['z']]
            self.src_x.append(source_coordinates[0])            # x coordinates of sources
            self.src_y.append(source_coordinates[1])            # y coordinates of sources
            self.src_z.append(source_coordinates[2])            # z coordinates of sources

        self.mic_x = []
        self.mic_y = []
        self.mic_z = []
        for i in d['Microphones']['ID']:
            microphone_coordinates = [d['Microphones']['microphones'][i]['parameters']['x'],
                                      d['Microphones']['microphones'][i]['parameters']['y'],
                                      d['Microphones']['microphones'][i]['parameters']['z']]
            self.mic_x.append(microphone_coordinates[0])            # x coordinates of microphones
            self.mic_y.append(microphone_coordinates[1])            # y coordinates of microphones
            self.mic_z.append(microphone_coordinates[2])            # z coordinates of microphones

        # room coordinates (height, length, width)
        room_coordinates = [d['Room']['height'], d['Room']['length'], d['Room']['width']]

        return {'src_x': self.src_x,
                'src_y': self.src_y,
                'src_z': self.src_z,
                'mic_x': self.mic_x,
                'mic_y': self.mic_y,
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # room canvas
        self.canvas = _Widget(data_file='Initial_configs.yaml')
...
class SourceWindow(QWidget):
    ...
    def source_selected_add(self, current_ind: int, data=None):
        ...
        if current_ind > 0:
            prev_source_ind = int(
                self.sources[current_ind - 1][6:])  # the index, that is wrote after Source, eg.Source3
...
class MicrophoneWindow(QWidget):
    ...
    def mic_selected_add(self, current_ind: int, data=None):
        ...
        if current_ind > 0:
            prev_mic_ind = int(
                self.mics[current_ind - 1][10:])  # the index, that is wrote after Microphone, eg.Microphone3
            print(prev_mic_ind)
